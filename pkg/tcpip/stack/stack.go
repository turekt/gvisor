...
// Returns the previous configuration on the NIC.
//
// TODO(https://fxbug.dev/96216): Implement support for multicast forwarding.
// Currently, setting this value is a no-op and is not ready for use.
...
type NICInfo struct {
	...
	// MulticastForwarding holds the forwarding status for each network endpoint that
	// supports multicast forwarding.
...
func (s *Stack) NICInfo() map[tcpip.NICID]NICInfo {
	...
	for id, nic := range s.nics {
		...
		for proto := range s.networkProtocols {
			if forwarding, ok := forwardingValue(nic.forwarding, proto, nic.ID(), "forwarding"); ok {
				info.Forwarding[proto] = forwarding
			}

			if multicastForwarding, ok := forwardingValue(nic.multicastForwarding, proto, nic.ID(), "multicastForwarding"); ok {
